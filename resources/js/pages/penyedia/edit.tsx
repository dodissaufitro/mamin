    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="mx-auto max-w-lg p-4 md:p-6">
                <h1 className="mb-6 text-lg font-bold text-violet-800 dark:text-violet-200">Edit Penyedia</h1>
                <form onSubmit={handleSubmit} className="rounded-xl border border-violet-200 bg-white p-5 shadow-sm dark:bg-sidebar dark:border-violet-800 flex flex-col gap-4">
                    <div className="flex flex-col gap-1">
                        ...
                        <input type="text" className={inputClass} value={data.nama} onChange={e => setData('nama', e.target.value)} />
                        {errors.nama && <p className="text-xs text-red-500">{errors.nama}</p>}
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className="text-sm font-semibold text-violet-700 dark:text-violet-300">Alamat</label>
                        <textarea rows={2} className={inputClass} value={data.alamat} onChange={e => setData('alamat', e.target.value)} />
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <input type="text" className={inputClass} value={data.telepon} onChange={e => setData('telepon', e.target.value)} />
